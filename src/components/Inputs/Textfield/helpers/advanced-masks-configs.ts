...
import { AdvancedMaskType, BeforeMaskedValueChangeTypeArgs } from "../../_inputsComponents/StandAloneTextfield/types";
...
//! TIME mask handler
const handleTimeBeforeChange = ({ newState, formatChars }: BeforeMaskedValueChangeTypeArgs): InputState => {
...
//! DATE mask handler
const handleDateBeforeChange = ({ newState, formatChars }: BeforeMaskedValueChangeTypeArgs): InputState => {
  const { value: newValue, selection: newSelection } = newState;
...
//! RGB mask handler
const handleRGBBeforeChange = ({ newState }: BeforeMaskedValueChangeTypeArgs): InputState => {
